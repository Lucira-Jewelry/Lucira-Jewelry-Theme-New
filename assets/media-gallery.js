...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const createDotsNavigation = () => {
    ...
    dotsContainer = document.createElement("div"); dotsContainer.className="custom-slider-dots";
    slides.forEach((_,i)=>{
  ...
  const goToSlide = (index) => {
    if(!mediaList) return; currentSlide=index;
    const slides = Array.from(mediaList.querySelectorAll(".product__media-item")).filter(s=>s.style.display!=="none");
  ...
  const getSelectedColor = () => {
    const inputs=document.querySelectorAll('input[name*="Color"]:checked,select[name*="Color"],input[name*="color"]:checked,select[name*="color"]');
    for(const input of inputs){ const c=getColorFromAlt(input.value); if(c) return c; }
    const urlColor=new URLSearchParams(window.location.search).get("color"); if(urlColor) return getColorFromAlt(urlColor);
    return "yellow";
  };
  ...
  const handleColorChange = debounce(()=>{
    ...
    setTimeout(()=>safeReorderByColor(selectedColor),50);
  },50);
  ...
  const initialize = () => {
    ...
    bodyObserver=new MutationObserver(()=>safeReorderByColor(currentSelectedColor));
    bodyObserver.observe(document.body,{childList:true,subtree:true});

    // Delegated drawer close
    document.body.addEventListener("click", e=>{
      if(e.target.closest("#customize_close_drawer")) restartActiveVideo();
    });

    ["change","variant:change","variant:selected","popstate"].forEach(evt=>document.addEventListener(evt, handleColorChange));
    ["variant:change","variant:selected"].forEach(evt=>document.addEventListener(evt,restartActiveVideo));
    document.addEventListener("shopify:section:load", ()=>{ currentSelectedColor=null; initialize(); });
