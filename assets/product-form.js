...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function markUserInteraction() {
        ...
        // Try to play videos after first interaction
        playAllVideos();
    }
    ...
    document.addEventListener('change', function(e) {
        if (e.target.matches('input[name*="Color"]') || e.target.matches('input[type="radio"][name*="option"]')) {
            setTimeout(function() {
                initializeVideos();
            }, 100);
        }
    ...
    function initializeVideos() {
        ...
        deferredMediaElements.forEach(function(deferredMedia) {
            const poster = deferredMedia.querySelector('.deferred-media__poster');
            const template = deferredMedia.querySelector('template');
            
            if (poster && template && !deferredMedia.classList.contains('loaded')) {
                const templateContent = template.content.cloneNode(true);
                const video = templateContent.querySelector('video');
                ...
                if (video) {
                    ...
                    // Set low volume instead of muted for better compatibility
                    video.volume = 0;
                    
                    // Only set autoplay if user has interacted or on desktop
                    if (userInteracted || !isMobile()) {
                        video.setAttribute('autoplay', 'autoplay');
                    }
                    
                    poster.style.display = 'none';
                    deferredMedia.appendChild(templateContent);
                    deferredMedia.classList.add('loaded');
                    ...
                    video.addEventListener('loadedmetadata', function() {
                        if (userInteracted || !isMobile()) {
                            video.play().catch(function(error) {
                                ...
                                showVideoFallback(deferredMedia, video, poster);
                            });
                        } else {
                            // Show poster with play button for mobile without interaction
                            showVideoFallback(deferredMedia, video, poster);
                        }
                    });
                        ...
                        const observer = new IntersectionObserver(function(entries) {
                            entries.forEach(function(entry) {
                                if (entry.isIntersecting) {
                                    ...
                                    if (video.paused && (userInteracted || !isMobile())) {
                                        video.play().catch(function(error) {
                                            console.log('Intersection video play failed:', error);
                                        });
                                    }
                                }
                            });
    ...
    function showVideoFallback(deferredMedia, video, poster) {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
        ...
        // Handle iOS focus events
        window.addEventListener('focus', function() {
            if (userInteracted) {
                setTimeout(playAllVideos, 300);
            }
        });
    }
    ...
    // Retry mechanism for stubborn videos
    let retryCount = 0;
    const maxRetries = 3;
    
    function retryVideoLoad() {
        if (retryCount < maxRetries && userInteracted) {
            retryCount++;
            setTimeout(function() {
                const pausedVideos = document.querySelectorAll('video:not([autoplay])');
    ...
    // Start retry mechanism after first interaction
