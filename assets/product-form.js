...
// Simple, reliable method to update drawer price in rupees
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateDrawerPrice() {
    ...
    try {
      ...
      if (variant && variant.price) {
        ...
        const priceInRupees = variant.price / 100;
        const formattedPrice = '₹' + (priceInRupees % 1 === 0 ? priceInRupees.toString() : priceInRupees.toFixed(2));
        ...
        // Visual feedback
        drawerPrice.style.transition = 'all 0.2s ease';
  ...
  // Watch for any changes in the variant drawer
  ...
  // Backup: Watch the variant script for changes
  const variantScript = document.querySelector('script[data-selected-variant]');
  if (variantScript) {
