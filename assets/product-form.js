...
document.addEventListener("DOMContentLoaded", function () {
    ...
    priceBreakupElement.addEventListener("click", function () {
      const metalPrice = document.getElementById("metal-price")?.textContent?.trim() || "";
      const diamondPrice = document.getElementById("diamond-price")?.textContent?.trim() || "";
      const makingPrice = document.querySelector("#making-table tr:last-child .price-value")?.textContent?.trim() || "";
      const gstRow = Array.from(document.querySelectorAll("#total-table tr"))
        .find(row => row.textContent.includes("GST"));
      const gstPrice = gstRow ? gstRow.querySelector("td.align-end")?.textContent?.trim() : "";

      const productData = {
        sku: "{{ product.selected_or_first_available_variant.sku }}",  // Shopify Liquid
        product_name: "{{ product.title }}",
        product_id: "{{ product.id }}",
        product_url: "{{ product.url }}",
        product_image: "https://lucirajewelry.com/cdn/shop/{{ product.featured_image }}",
        creative_name: "priceBreakup",
        variant_id: "{{ product.selected_or_first_available_variant.id }}",
        price: "{{ product.selected_or_first_available_variant.compare_at_price | money_without_currency }}",
        offer_price: "{{ product.selected_or_first_available_variant.price  | money_without_currency}}", 
        metal_price: metalPrice,
        diamond_price: diamondPrice,
        making_price: makingPrice,
        gst_price: gstPrice
      };
