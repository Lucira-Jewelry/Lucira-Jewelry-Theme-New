...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!variantSelectsDrawer || !container) return;

  const API_URL = "https://app.lucirajewelry.com/inventory/inventory-check.php";
  ...
  function renderInventory(data) {
    if (!data) {
      container.innerHTML = `<div class="store-loading">Checking availability...</div>`;
      return;
    }

    if (data.in_stock_locations && data.in_stock_locations.length > 0) {
      const store = data.in_stock_locations[0];
      ...
      container.innerHTML = `
        <div class="store-available">
          ...
          ${
            data.in_stock_locations.length > 1
              ? `<p class="extra-stores">+ ${data.in_stock_locations.length - 1} other stores</p>`
              : ""
          }
        </div>
      `;
      ...
      container.innerHTML = `
        <div class="made-to-order-box">
          Dispatch by: ${getFutureDate()} | This item is made to order
        </div>
      `;
    }
  }
  ...
  function fetchInventory(variantId) {
    ...
    fetch(`${API_URL}?variant_id=${variantId}`)
      ...
      .catch((err) => {
        ...
        renderInventory({ in_stock_locations: [] });
      });
  }

  // ✅ INITIAL LOAD (drawer open case)
  const input = document.querySelector(
    '#variant-drawer form[action="/cart/add"] input[name="id"]'
  );
  if (input && input.value) {
    fetchInventory(input.value);
  }

  // ✅ SAME PATTERN AS YOUR PRICE CODE
  variantSelectsDrawer.addEventListener("variant:change", function (event) {
    const variant = event.detail?.variant;
    if (!variant) return;

    fetchInventory(variant.id);
  });
