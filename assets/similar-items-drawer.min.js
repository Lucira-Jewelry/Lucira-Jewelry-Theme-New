...
// When page loads, find the first product's button
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (productCards.length > 0) {
    ...
    // Find the button within the first product card
    const firstButton = firstProductCard.querySelector('.view-similar-btn');
    
    if (firstButton) {
      // Add the animation class only to the first product's button
      const textElement = firstButton.querySelector('.view-similar-btn__text');
      if (textElement) {
        textElement.classList.add('first-load');
        
        // Remove the class after animation completes to prevent re-triggering
        setTimeout(() => {
          textElement.classList.remove('first-load');
