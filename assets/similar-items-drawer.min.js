...
document.addEventListener('DOMContentLoaded', function() {
  // Get ALL buttons with first-load class
  const allFirstLoadButtons = document.querySelectorAll('.view-similar-btn.first-load');
  
  console.log('Found buttons with first-load:', allFirstLoadButtons.length);
  
  // Remove first-load class from ALL buttons
  allFirstLoadButtons.forEach(button => {
    button.classList.remove('first-load');
  });
  
  // Find only the first product card's button
  const firstProductCard = document.querySelector('.product-card');
  
  if (firstProductCard) {
    const firstButton = firstProductCard.querySelector('.view-similar-btn');
    
    if (firstButton) {
      // Add first-load class only to the first button
      firstButton.classList.add('first-load');
      
      console.log('Added first-load to first button');
      
      // Remove after animation completes
      setTimeout(() => {
        firstButton.classList.remove('first-load');
      }, 2000);
    }
  }
});