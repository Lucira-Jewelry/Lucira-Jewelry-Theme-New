...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (productCards.length > 0) {
    // Get only the FIRST product card
    const firstCard = productCards[0];
    
    // Find its button
    const firstButton = firstCard.querySelector('.view-similar-btn');
    
    if (firstButton) {
      // Add animation class
      firstButton.classList.add('first-load');
      
      // Optional: Force animation through inline style
      const textSpan = firstButton.querySelector('.view-similar-btn__text');
      if (textSpan) {
        textSpan.style.animation = 'slideInText 2s ease-in-out';
