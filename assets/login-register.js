...
if (sendBtn) {
  sendBtn.addEventListener('click', async () => {
    ...
    // Handle 10 digit input by adding 91
    ...
    sendBtn.textContent = 'Sending...'; // Optional: Give feedback on button
    ...
    try {
      ...
      // 1. Check if the server actually responded with success (200 OK)
      if (!response.ok) {
        throw new Error(`Server Error: ${response.status}`);
      }

      // 2. Read text first to prevent JSON parse crash
      ...
      try {
        ...
        // This catches the 5% where server returns HTML (500/502/504 errors)
        console.error('Server returned non-JSON:', textResponse);
        throw new Error('Server currently unavailable. Please try again later.');
      }

      // 3. Handle Logical API Errors (e.g., "SMS limit reached")
      ...
      const errorMsg = e.message.includes('Server currently unavailable') 
      ...
      sendBtn.textContent = 'SEND OTP'; // Reset button text
