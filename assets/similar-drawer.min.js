const SimilarItemsDrawer = (() => {
  ...
  function getPageType() {
    ...
    // Check for Shopify template classes
    if (bodyClasses.includes('template-index') || path === '/') {
      return 'homepage';
    } else if (bodyClasses.includes('template-collection') || path.includes('/collections/')) {
      return 'plp';
    } else if (bodyClasses.includes('template-product') || path.includes('/products/')) {
      return 'pdp';
    } else if (bodyClasses.includes('template-search') || path.includes('/search')) {
      return 'search';
    }

    // Fallback: check URL structure
    if (path === '/' || path === '/index') return 'homepage';
    if (path.match(/^\/collections\//)) return 'plp';
    if (path.match(/^\/products\//)) return 'pdp';

    return 'other';
  }

  // Helper function to extract product name from button
  function getProductName(button) {
    // Try to get from aria-label first
    const ariaLabel = button.getAttribute('aria-label');
    if (ariaLabel) {
      const match = ariaLabel.match(/View similar items for (.+)/i);
      if (match && match[1]) {
        return match[1];
      }
    }

    // Try to get from data attribute if added
    if (button.dataset.productTitle) {
      return button.dataset.productTitle;
    }

    // Try to find product title in parent card
    const productCard = button.closest('.card, .product-card, [class*="product-"]');
    if (productCard) {
      const titleElement = productCard.querySelector('.card__title, .product-card__title, [class*="product-title"], [class*="card-title"]');
      if (titleElement) {
        return titleElement.textContent.trim();
      }
    }

    // Fallback to product handle if nothing else works
    return button.dataset.productHandle || 'Unknown Product';
  }

  // Push to dataLayer
function trackPromoClick(button, triggerSource) {
  window.dataLayer = window.dataLayer || [];

  const pageType = getPageType();
  const productName = getProductName(button);

  // Differentiate creative_name based on who triggered it
  const creativeName = triggerSource === 'exit-intent'
    ? 'Exit Intent - View Similar Drawer'
    : 'View Similar Button';
  ...
  window.dataLayer.push({
    ...
    promoClick: {
      ...
      // Only present on exit intent pushes
      ...(triggerSource === 'exit-intent' && { trigger_source: window.__exitIntentSource || 'unknown' })
    }
  });
  ...
  // Add this new function alongside trackPromoClick
function trackSimilarItemClick(gridItem) {
  window.dataLayer = window.dataLayer || [];

  const isExitIntent = state.lastTriggerSource === 'exit-intent';

  window.dataLayer.push({
    event: "promoClick",
    promoClick: {
      creative_name: isExitIntent
        ? "Exit Intent - Similar Item Product Clicked"
        : "Similar Items Product Clicked",
      promo_name: gridItem.dataset.productTitle || "Unknown Product",
      promo_position: window.location.href,
      location_id: getPageType(),
      ...(isExitIntent && { trigger_source: window.__exitIntentSource || 'unknown' })
    }
  });
}
  ...
  function createDrawer() {
    ...
    span.className = "overlay-inner-span"; // optional class
...
function handleClick(event) {
  ...
  if (button) {
    ...
    // ✅ Read flag set by exit intent script, fallback to 'manual'
    const triggerSource = window.__drawerTriggerSource === 'exit-intent' ? 'exit-intent' : 'manual';
    trackPromoClick(button, triggerSource);
    state.lastTriggerSource = triggerSource;
    // ✅ Reset flags immediately after reading
    window.__drawerTriggerSource = null;
    window.__exitIntentSource = null;
  ...
  // Track clicks on products inside the drawer
  if (state.isOpen) {
    const gridItem = event.target.closest(".similar-items-grid__item");
    if (gridItem) {
      trackSimilarItemClick(gridItem);
    }
  }
