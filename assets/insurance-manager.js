...
(function() {
  ...
  async function fetchInsurancePrice() {
    if (state.priceFetched && state.insurancePrice !== null) {
      return state.insurancePrice;
    }
    
    try {
      log('💰', 'Fetching insurance price from Shopify product...');
      
      // Method 1: Try to get from variant endpoint
      try {
        const variantResponse = await fetch(`/products/${CONFIG.VARIANT_ID}.js`);
        if (variantResponse.ok) {
          const variantData = await variantResponse.json();
          if (variantData.price) {
            state.insurancePrice = variantData.price;
            state.priceFetched = true;
            log('✅', 'Price fetched from variant endpoint:', formatMoney(state.insurancePrice));
            updatePriceDisplay();
            return state.insurancePrice;
          }
        }
      } catch (e) {
        log('⚠️', 'Variant endpoint failed, trying alternative...');
      }
      
      // Method 2: Try to get from cart if already added
      const cartResponse = await fetch('/cart.js');
      const cart = await cartResponse.json();
      const insuranceItem = cart.items.find(item => 
        String(item.variant_id) === CONFIG.VARIANT_ID || 
      ...
      // Method 3: Try fetching product handle and then product.js
      try {
        const productsResponse = await fetch('/products.json?limit=250');
        if (productsResponse.ok) {
          const productsData = await productsResponse.json();
          const insuranceProduct = productsData.products.find(product => 
            product.variants.some(v => String(v.id) === CONFIG.VARIANT_ID)
          );
          
          if (insuranceProduct) {
            const variant = insuranceProduct.variants.find(v => String(v.id) === CONFIG.VARIANT_ID);
            if (variant && variant.price) {
              state.insurancePrice = parseInt(variant.price) * 100; // Convert to cents
              state.priceFetched = true;
              log('✅', 'Price fetched from products.json:', formatMoney(state.insurancePrice));
              updatePriceDisplay();
              return state.insurancePrice;
            }
          }
        ...
        log('⚠️', 'Products.json method failed');
      }
      
      throw new Error('Unable to fetch insurance price from any source');
      
    } catch (error) {
      log('❌', 'Price fetch error:', error);
      // Fallback to 10000 (₹100) if unable to fetch
      state.insurancePrice = 10000;
      state.priceFetched = true;
      log('⚠️', 'Using fallback price:', formatMoney(state.insurancePrice));
      updatePriceDisplay();
      return state.insurancePrice;
    }
  }
  ...
  function updatePriceDisplay() {
    ...
    const priceElements = document.querySelectorAll('[data-insurance-price]');
    priceElements.forEach(el => {
      el.textContent = formatMoney(state.insurancePrice);
    });
    
    log('💵', 'Price display updated:', formatMoney(state.insurancePrice));
  }
  ...
  async function addInsuranceToCart(quantity) {
    ...
    try {
      ...
      // Update price from response if not already fetched
      if (!state.priceFetched && result.items && result.items.length > 0) {
        const insuranceItem = result.items.find(item => 
          String(item.variant_id) === CONFIG.VARIANT_ID
        );
        if (insuranceItem && insuranceItem.price) {
    ...
    if (elements.checkbox) {
      ...
      if (label) {
        ...
        if (hasInsuranceInCart && insuranceQuantity > 0) {
          ...
          // Show single unit price when not added
          const priceElement = label.querySelector('.insurance-price');
          if (priceElement && state.insurancePrice) {
            priceElement.textContent = formatMoney(state.insurancePrice);
          }
        }
      }
    }
      ...
      for (const mutation of mutations) {
        ...
        if (mutation.type === 'attributes' && mutation.attributeName === 'open') {
          ...
          if (target.tagName === 'CART-DRAWER' && target.hasAttribute('open')) {
            setTimeout(async () => {
              attachEventListeners();
              // Fetch price if not already fetched when cart opens
              if (!state.priceFetched) {
                await fetchInsurancePrice();
              } else {
                updatePriceDisplay();
  ...
  async function init() {
    log('🚀', 'Initializing Insurance Manager v7.1...');
    
    // Fetch insurance price immediately on init
    await fetchInsurancePrice();
    ...
    state.initialized = true;
    log('✅', 'Insurance Manager initialized with price:', formatMoney(state.insurancePrice || 0));
  }
