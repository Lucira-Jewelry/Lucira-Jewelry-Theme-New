...
(function() {
  ...
  async function updateInsuranceSectionVisibility(cartData) {
    ...
    if (insuranceWrapper) {
      if (hasProducts) {
        ...
        // If insurance exists but no products, remove it and show empty cart
        if (hasInsuranceInCart) {
          log('⚠️', 'Removing insurance - no products in cart');
          try {
            state.processing = true;
  ...
  async function syncInsuranceWithProducts() {
    ...
    try {
      ...
      // If no products left, remove insurance and show empty cart
      if (totalNonInsuranceQuantity === 0) {
        log('⚠️', 'No products left, removing insurance and showing empty cart');
    ...
    if (typeof subscribe === 'function' && typeof PUB_SUB_EVENTS !== 'undefined') {
      subscribe(PUB_SUB_EVENTS.cartUpdate, (event) => {
        ...
        setTimeout(async () => {
          ...
          // Check if cart is empty (only insurance left)
          const cartData = await getCart();
          if (cartData) {
            const nonInsuranceCount = getNonInsuranceCount(cartData);
            if (nonInsuranceCount === 0 && hasInsurance(cartData)) {
              log('⚠️', 'Cart empty except insurance - triggering cleanup');
              await removeInsuranceFromCart();
              await wait(300);
