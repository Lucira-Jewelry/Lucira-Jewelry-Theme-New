...
(function() {
  ...
  async function removeInsuranceThenProduct(lineItemKey) {
  log('🔥', 'Intercepted last product removal');

  state.processing = true;
  showLoader(true);

  // 1️⃣ Remove insurance FIRST
  if (hasInsurance(await getCart())) {
    await removeInsuranceFromCart();
    await wait(400);
  }

  // 2️⃣ Now remove product
  ...
  async function isLastNonInsuranceProduct(lineItemKey) {
  const cart = await getCart();
  if (!cart) return false;

  const nonInsuranceItems = cart.items.filter(
    item =>
      String(item.variant_id) !== CONFIG.VARIANT_ID &&
      String(item.id) !== CONFIG.VARIANT_ID
  );

  // If only ONE non-insurance item AND it's the one being removed
  return nonInsuranceItems.length === 1 &&
         nonInsuranceItems[0].key === lineItemKey;
}
  ...
  const lineItemKey = removeBtn.getAttribute('data-line-item-key');
  if (!lineItemKey) return;

  const isLast = await isLastNonInsuranceProduct(lineItemKey);
  if (!isLast) return;
  ...
  await removeInsuranceThenProduct(lineItemKey);
