...
(function() {
  ...
  async function updateInsuranceQuantity(quantity) {
    ...
    if (totalNonInsuranceQuantity === 0 && hasInsuranceInCart) {
      log('🔥', 'Last product removed → removing insurance FIRST');
    ...
    const cartObserver = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        ...
        quantityInputs.forEach(input => {
          ...
          const stopped = await removeInsuranceBeforeLastProduct();
          if (!stopped) {
            syncInsuranceWithProducts();
          }
        }, 500));
  ...
  async function syncInsuranceWithProducts() {
    ...
    try {
      ...
      const stop = await removeInsuranceBeforeLastProduct();
      if (stop) return;
