...
(function() {
  ...
  console.log('🎯 Insurance Manager v5.2 Starting...');
  ...
  async function forceCartRefresh() {
    ...
    try {
      ...
      if (newFooter && currentFooter) {
        currentFooter.replaceWith(newFooter);
        log('✅', 'Cart footer replaced');
      }
      
      // Replace checkout button section
      const newCheckoutButton = doc.querySelector('.checkout-button');
      const currentCheckoutButton = document.querySelector('.checkout-button');
      
      if (newCheckoutButton && currentCheckoutButton) {
        currentCheckoutButton.replaceWith(newCheckoutButton);
        log('✅', 'Checkout button replaced');
      }
      ...
      if (cartDrawer && freshCart) {
        ...
        await updateGrandTotalUI(freshCart);
        await syncCheckboxState();
      }
      
      // Re-attach event listeners
      attachEventListeners();
      
      log('✅', 'Cart UI fully refreshed');
    } catch (error) {
      log('❌', 'Force refresh error:', error);
    }
  }
  ...
  function setupQuantitySync() {
    ...
    const cartObserver = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        ...
        // Detect when cart items are removed from DOM
        if (mutation.removedNodes.length > 0) {
          mutation.removedNodes.forEach(node => {
            if (node.nodeType === 1 && 
                (node.matches && node.matches('cart-item') || 
                 node.classList && node.classList.contains('cart-item'))) {
              log('🗑️', 'Cart item removed from DOM');
              setTimeout(() => {
    ...
    // Also listen for quantity buttons and remove buttons
    document.addEventListener('click', (event) => {
      const target = event.target;
      
      // Quantity buttons
      if (target.matches('.cart-item__quantity-wrapper button') || 
          target.closest('.cart-item__quantity-wrapper button')) {
        log('🔄', 'Quantity button clicked');
        setTimeout(() => {
          syncInsuranceWithProducts();
        }, 300);
      ...
      // Remove buttons - trigger immediate check
      if (target.matches('cart-remove-button') || 
          target.closest('cart-remove-button') ||
          target.matches('.cart-item__remove') ||
          target.closest('.cart-item__remove')) {
        log('🗑️', 'Remove button clicked');
        
        // Multiple checks to ensure we catch the removal
        setTimeout(() => handleProductRemoval(), 100);
        setTimeout(() => handleProductRemoval(), 300);
        setTimeout(() => handleProductRemoval(), 500);
      }
    });
  }
  
  async function handleProductRemoval() {
    log('🗑️', 'Handling product removal...');
    
    // Prevent multiple simultaneous executions
    if (state.processing) {
      log('⏳', 'Already processing removal, skipping...');
      return;
    }
    ...
    try {
      ...
      log('📊', 'After removal - Products:', totalNonInsuranceQuantity, 'Insurance:', hasInsuranceInCart);
      
      // If no products left but insurance still exists
      if (totalNonInsuranceQuantity === 0 && hasInsuranceInCart) {
        log('⚠️', 'Last product removed, removing insurance immediately...');
        state.processing = true;
        showLoader(true);
        ...
        log('✅', 'Insurance removed and cart refreshed');
      } else if (hasInsuranceInCart && totalNonInsuranceQuantity > 0) {
        // Sync insurance quantity with remaining products
        await syncInsuranceWithProducts();
      }
    } catch (error) {
      log('❌', 'Product removal handling error:', error);
      state.processing = false;
      showLoader(false);
    }
  }
  ...
  function listenToCartEvents() {
    ...
    if (typeof subscribe === 'function' && typeof PUB_SUB_EVENTS !== 'undefined') {
      subscribe(PUB_SUB_EVENTS.cartUpdate, async (event) => {
        ...
        log('📊', 'After cart update - Products:', totalNonInsuranceQuantity, 'Insurance:', hasInsuranceInCart);
