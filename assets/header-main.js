...
// register user datalayer
(function () {
  function bindRegisterTrack() {
    const updateBtn = document.querySelector(".update-btn");
    if (!updateBtn || updateBtn.dataset.trackBound) return;

    updateBtn.dataset.trackBound = "true";
  ...
  new MutationObserver(bindRegisterTrack).observe(document.body, {
    childList: true,
    subtree: true,
  });
})();

//login user datalayer
(function () {
  ...
  function observeSteps() {
    ...
    // If update-user-box ever becomes visible → it's a new user
    if (updateBox && !updateBox.classList.contains("hideBox")) {
      updateBoxShown = true;
    }

    // OTP step finished & update box never shown → LOGIN
    if (
      verifyBox &&
  ...
  const observer = new MutationObserver(observeSteps);
